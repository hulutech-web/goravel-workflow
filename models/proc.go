...
type Proc struct {
	...
	Status                int              `gorm:"column:status;not null;comment:'当前处理状态 0待处理 1已通过 9会签通过 -1驳回 -2撤回 3转交 4跳过'" json:"status" form:"status"`
	...
	UnpassTargetProcessID int              `gorm:"column:unpass_target_process_id;not null;default:0;comment:'驳回到指定节点的目标步骤ID'" json:"unpass_target_process_id" form:"unpass_target_process_id"`
	...
	AddedSigns            []ProcAddSign    `gorm:"foreignkey:ProcID;constraint:OnUpdate:CASCADE,OnDelete:NO ACTION"`  // HasMany ProcAddSign
}
